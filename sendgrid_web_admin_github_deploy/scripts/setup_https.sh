...
#
# This process normally stays alive on Ubuntu 24.04
# and does not mean apt/dpkg is locked.
# ------------------------------------------------------------
apt_is_busy() {
  local lock_file
...
if curl \
  ...
  echo "WARNING:"
  echo "Cannot access:"
  echo "http://127.0.0.1:${APP_PORT}/api/health"
  echo ""
  echo "继续配置 Nginx 和 HTTPS。"
  echo ""
  echo "请确认项目运行在 ${APP_PORT} 端口。"
  echo ""
  echo "如果后续出现 502，请执行："
  echo ""
...
if command -v ufw >/dev/null 2>&1; then
  ...
  ufw delete allow "${APP_PORT}/tcp" \
    >/dev/null 2>&1 || true

fi

# ------------------------------------------------------------
# Ensure the backend is only reachable through Nginx.
# ------------------------------------------------------------
if [ -f "${APP_DIR}/.env" ]; then

  if grep -q '^SERVER_HOST=' "${APP_DIR}/.env"; then
    sed -i \
      's/^SERVER_HOST=.*/SERVER_HOST=127.0.0.1/' \
      "${APP_DIR}/.env"
  else
    echo "SERVER_HOST=127.0.0.1" >> "${APP_DIR}/.env"
  fi

  if grep -q '^SESSION_COOKIE_SECURE=' "${APP_DIR}/.env"; then
    sed -i \
      's/^SESSION_COOKIE_SECURE=.*/SESSION_COOKIE_SECURE=true/' \
      "${APP_DIR}/.env"
  else
...
if [ -f "${SERVICE_FILE}" ]; then
  ...
  echo ""
  echo "Restricting backend service to 127.0.0.1..."
...
echo ""
echo "Testing Nginx after certificate installation..."
...
if curl \
  ...
  HTTPS_STATUS="CHECK_DOMAIN_DNS_OR_NGINX"
